...
#import "ZHLinkLabel.h"

@interface ZHLinkLabel ()<UITextViewDelegate>
/**
 *  存储点击事件的数组
 */
 ...
 *  @param item 传入点击事件
 */
- (void)addClickItem:(ZHLinkLabelClickItem *)item{
...
- (void)addClickText:(NSString *)text attributeds:(NSDictionary *)attributeds transmitBody:(id)transmitBody clickItemBlock:(ZHLinkLabelClickItemBlock)clickBlock{
    // 根据现有的文本生成可变的富文本
    ...
    if (range.location != NSNotFound) {
        ...
        ZHLinkLabelClickItem *item = [ZHLinkLabelClickItem itemWithText:text range:range transmitBody:transmitBody];
...
- (void)addClickTextAttachmentName:(NSString *)attachmentName TransmitBody:(id)transmitBody clickItemBlock:(ZHLinkLabelClickItemBlock)clickBlock{
    ZHLinkLabelClickItem *item = [ZHLinkLabelClickItem itemWithTransmitBody:transmitBody];
    item.clickBlock = clickBlock;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    // 遍历所有的点击事件
    [self.clickItems enumerateObjectsUsingBlock:^(ZHLinkLabelClickItem *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        // 遍历点击事件里的点击范围
    ...
    if (!isClickText) {
        if (self.attributedText) {
            ...
            [self.attributedText enumerateAttribute:NSAttachmentAttributeName inRange:NSMakeRange(0, self.attributedText.length) options:NSAttributedStringEnumerationReverse usingBlock:^(ZHTextAttachment* value, NSRange range, BOOL * _Nonnull stop) {
                if (value && CGRectEqualToRect(value.bounds, CGRectMake(0, 0, 23, 23))) {
                    ...
                    if (CGRectContainsPoint(rect, point)) {
                        ZHLinkLabelClickItem *item = [self.clickAttachmentItems valueForKey:value.attachmentName];
                        if (item) {
                            if (item.clickBlock) {
...
@implementation ZHLinkLabelClickItem

+ (instancetype)itemWithText:(NSString *)text range:(NSRange)range transmitBody:(id)transmitBody{
    
    ZHLinkLabelClickItem *item = [[ZHLinkLabelClickItem alloc] init];
