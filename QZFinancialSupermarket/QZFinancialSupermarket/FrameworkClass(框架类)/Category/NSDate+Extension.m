...
+ (NSString *)dateFormatter:(NSString *)formatter dateStr:(NSString *)dateStr{
    ...
    rang.location = 0;
    rang.length = 10;
    NSString *needtime = [dateStr substringWithRange:rang];
    
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:[needtime doubleValue]];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:formatter];
...
+ (NSString *)dateStr:(NSString *)dateStr{
    return [self dateFormatter:@"yyyy年MM月dd日" dateStr:dateStr];
}
